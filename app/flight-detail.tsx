...
export default function FlightDetailScreen() {
    ...
    const item = useAppSelector((state) => state.selectFlight.item)
    ...
    function buildTips(legItem: Leg) {
        ...
        if (placeholders.includes('{{ATD}}')) {
            placeholders = placeholders.replace('{{ATD}}', getTimeFromDate(legItem.arrival.scheduleTime.date))
        }
        if (placeholders.includes('{{ETA}}')) {
            placeholders = placeholders.replace('{{ETA}}', getTimeFromDate(legItem.departure.latestUpdatedTime.date))
        }
        if (placeholders.includes('{{ETD}}')) {
            placeholders = placeholders.replace('{{ETD}}', getTimeFromDate(legItem.departure.scheduleTime.date))
        }
        return placeholders
    }
                        ...
                        <View
                            ...
                            <View style={styles.flightMarkContainer}>
                                <View style={styles.airportMark}>
                                    <Text
                                        style={styles.airportCode}>{legItem.departure.airport ?? '--'}</Text>
